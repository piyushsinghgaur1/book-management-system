      ...
      .then(data => {
        const fetchedBooks = data.items.map(item => {
          ...
          return {
            ...
            volumeInfo.industryIdentifiers[0].identifier  : `API-${Math.random().toString(36).substr(2, 9)}`,
            publicationDate: volumeInfo.publishedDate || 'N/A',
            price: saleInfo.saleability === "FOR_SALE" && saleInfo.listPrice
            ? saleInfo.listPrice.amount || saleInfo.listPrice.amountInMicros: saleInfo.saleability, 
...
apiBooksManager.fetchBooksFromAPI();